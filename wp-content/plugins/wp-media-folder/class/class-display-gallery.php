...
class WpmfDisplayGallery
{
    ...
    public function gallery($attr)
    {
        ...
        if (isset($wpmf_autoinsert) && ((int)$wpmf_autoinsert === 1 || $wpmf_autoinsert === 'on') && !empty($wpmf_folder_id)) {
            ...
            $args = array(
                ...
                'post_mime_type' => array('image/jpeg', 'image/gif', 'image/png', 'image/bmp', 'image/tiff', 'image/x-icon', 'image/webp', 'application/pdf'),
                'order' => $wpmf_order,
    ...
    public function getAttachmentLink(
        ...
        if ($size && 'none' !== $size) {
            ...
            if (!empty($drive_id)) {
                $text = '<img class="wpmf_img wpmf_img_cloud" src="'. wp_get_attachment_image_url($id, $size) .'" data-type="wpmfgalleryimg" data-lazy-src="0">';
            } else {
                $text = '<img class="wpmf_img" src="'. wp_get_attachment_image_url($id, $size) .'" data-type="wpmfgalleryimg" data-lazy-src="0">';
            }
        } else {
            $text = '';
        }
